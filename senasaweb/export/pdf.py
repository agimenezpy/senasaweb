...
STILO = [('ALIGN',(0,0),(-1,0),'CENTER'),
         ...
         ('FONTSIZE',(0,0),(-1,-1),8)]
MEDIDAS = [1*inch,1.1*inch,1*inch,0.7*inch,0.7*inch,1.5*inch,0.8*inch,1.3*inch,1*inch,0.8*inch,0.8*inch]
...
class Caratula(Flowable):
    ...
    def draw(self):
        ...
        canvas.drawImage(join(self.filedir,"static/img/vivapy.png"),0,PAGE_WIDTH,width=2.63*cm,preserveAspectRatio=True)
        canvas.drawImage(join(self.filedir,"static/img/senasa.png"),PAGE_WIDTH/2-3*cm,PAGE_WIDTH+2.54*cm,width=2.73*cm,preserveAspectRatio=True)
        canvas.drawImage(join(self.filedir,"static/img/ministerio.png"),PAGE_WIDTH-6.5*cm,PAGE_WIDTH+1.27*cm,width=3.1*cm,preserveAspectRatio=True)
