...
from parametros.models import Tipo, Departamento, Proyecto, Grupo
...
def obras(request):
    ...
    rs = Obra.objects.select_related("distrito").only("locacion",
                                                      "proceso", "distrito__nombre", "distrito__departamento",
                                                      "producto", "cantidad",
...
def get_iconos():
    ...
    iconos = reduce(lambda i, v: i + u"," + v,
                    map(lambda it: u'"%s" : "%s"' % (it.etiqueta, it.color if it.color is not None else 'default'),
                        Tipo.objects.filter(categoria__exact="PRD")))
...
def get_deps():
    if cache.has_key("departamento"):
        ...
        DEPS = {}
        for rw in Departamento.objects.only("codigo", "nombre").extra(
                select={"extension": "box(ST_transform(geom,4326))"}).all():
